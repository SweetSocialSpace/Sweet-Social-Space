...
export default function ProfilePage(){
  ...
  return (
    <div className="min-h-screen w-full flex justify-center pt-8 pb-20">
      <div className="w-full max-w-2xl bg-black/30 backdrop-blur-xl rounded- border border-white/10 p-6 mx-4">
        <h1 className="text-2xl font-black text-white">Your Subscriber Profile</h1>
        <p className="text-white/60 text-sm mb-6">This is what neighbors in {profile.zip} see about you</p>
        ...
        {tab==='home' && (
          <div className="space-y-4">
            ...
            <textarea value={profile.bio} onChange={e=>setProfile({...profile, bio:e.target.value})} className="w-full bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-4 h-32 text-white placeholder:text-white/40 focus:bg-white/10 focus:outline-none resize-none" placeholder="Owner and creator" />
            ...
            <textarea value={profile.interests} onChange={e=>setProfile({...profile, interests:e.target.value})} className="w-full bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-4 h-24 text-white placeholder:text-white/40 focus:bg-white/10 focus:outline-none resize-none" placeholder="Interests, skills..." />
