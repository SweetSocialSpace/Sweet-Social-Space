      ...
      <div className="relative z-10 max-w-7xl w-full grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-10 items-center">
        <div className="text-left">
          ...
          <p className="mt-6 text-lg text-white/90 leading-relaxed font-semibold drop-shadow">
            Your neighbor might have free stuff. Another might needs a job. Someone 3 houses down just posted an alert.
          </p>
          <p className="mt-4 text-base text-white/70 leading-relaxed">
            Sweet Social Space lets you hear from your neighbors within 5,10.15,20 miles of YOU — wherever you are in the world. No robots deciding what you see. No shadowbans for your faith. Just real neighbors, right now.
          </p>
          <p className="mt-6 text-sm font-bold text-white/50 tracking-widest uppercase">
            Speak Freely. Love Your Neighbor. •Jesus LOVES You!-Independent
          </p>
        </div>
        ...
        <div className="w-full bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 p-8 shadow-2xl">
          ...
          <p className="text-white/80 text-center text-sm mb-6 mt-2 font-semibold">Your Neighbors missed you. </p>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-3 rounded-full">
              SIGN IN — See Your Neighborhood !
            </button>
          </form>
          {msg && <p className="mt-4 text-center text-sm text-white bg-white/10 p-2 rounded-lg">{msg}</p>}
          <p className="mt-6 text-center text-sm text-white/60">
            No account? <Link href="/signup" className="text-white font-bold underline">Join your Neighbors — free </Link>
          </p>
          <div className="text-white/20 uppercase tracking-widest text-center mt-4 text-sm">WE care about your privacy • FAILSAFE</div>
        </div>
      </div>
    </div>
  )
