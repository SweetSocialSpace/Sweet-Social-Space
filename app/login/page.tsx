...
export default function LoginPage() {
  ...
  const displayZip = (zip && zip.toUpperCase()!=='YOUR BLOCK' && zip!==''? zip : 'GLOBAL')
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // GLOBAL - ensure profile has zip - vertebrae failsafe
      if (data?.user) {
        try {
          const safeZip = displayZip === 'GLOBAL'? 'GLOBAL' : displayZip
          if (safeZip!== 'GLOBAL') {
            await supabase.from('profiles').update({ zip_code: safeZip, zip: safeZip, city: city || '', body: `Login in ${safeZip}` } as any).eq('id', data.user.id)
          }
        } catch {}
      }
      router.push('/feed')
      ...
      <div className="relative z-10 max-w-7xl w-full grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-10 items-center">
        <div className="text-left">
          <h1 className="text-5xl font-black text-white leading-tight drop-shadow-xl">
            Facebook shows you the world.<br/>We show you your block in {displayZip}.
          </h1>
          <p className="mt-6 text-lg text-white/90 leading-relaxed font-semibold drop-shadow">
            Your neighbor in {city || displayZip} has a free couch. Another needs a job. Someone 3 houses down just posted an alert. You missed it scrolling people 3,000 miles away.
          </p>
          <p className="mt-4 text-base text-white/70 leading-relaxed">
            Sweet Social Space is private to neighbors within 10 miles of YOU — wherever you are in the world — {displayZip} • GLOBAL • LIVE. No robots deciding what you see. No shadowbans for your faith. Just real neighbors, right now.
          </p>
          <p className="mt-6 text-sm font-bold text-white/50 tracking-widest uppercase">
            Speak Freely. Love Your Neighbor. • {displayZip} • GLOBAL • VERTEBRAE • Independent
          </p>
        </div>
        ...
        <div className="w-full bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 p-8 shadow-2xl">
          ...
          <p className="text-white/80 text-center text-sm mb-6 mt-2 font-semibold">Your block in {displayZip} missed you. {city} • GLOBAL • LIVE</p>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-3 rounded-full">
              SIGN IN — See Your Block in {displayZip}
            </button>
          </form>
          {msg && <p className="mt-4 text-center text-sm text-white bg-white/10 p-2 rounded-lg">{msg}</p>}
          <p className="mt-6 text-center text-sm text-white/60">
            No account? <Link href="/signup" className="text-white font-bold underline">Join your block in {displayZip} — free • GLOBAL</Link>
          </p>
          <div className="text-white/20 text- uppercase tracking-widest text-center mt-4">GLOBAL • {displayZip} • {city} • VERTEBRAE • FAILSAFE</div>
        </div>
      </div>
    </div>
  )
