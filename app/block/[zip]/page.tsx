  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="mt-6">
        {posts.length === 0 ? (
          <div className="p-8 text-center border border-dashed border-white/30 rounded-2xl bg-black/30">
            ...
            <a href={`/feed?zip=${zip}`} className="mt-4 inline-block bg-white text-black px-6 py-2 rounded-full font-bold">
              Post in {zip} →
            </a>
          </div>
        ) : (
      ...
      <div className="mt-8 flex gap-3">
        <a href={`/feed?zip=${zip}`} className="bg-white text-black px-6 py-3 rounded-full">Enter {zip} Feed →</a>
        <a href="/feed" className="border border-white text-white px-6 py-3 rounded-full">Change Zip</a>
      </div>
    </div>
  )
}
