...
export async function GET(req: Request) {
  try {
    ...
    data.forEach((p: any) => { const street = (p.location_text || zip).trim(); counts[street] = (counts[street] || 0) + 1; });
    const sorted = Object.entries(counts).sort((a,b) => b[1]-a[1]);
    const hottest = sorted[0];
    return NextResponse.json({ street: hottest[0], count: hottest[1], total: data.length, zip, time: new Date().toISOString() });
  } catch {
