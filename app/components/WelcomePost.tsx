...
export default function WelcomePost() {
  const { zip, city, isLive } = useLocation() as any
  // no Manado - use city only - fallback to area - never show Indonesia
  const area = city && city!== 'Manado' &&!city.includes('Sulawesi')? city : (zip && zip!== 'GLOBAL'? zip : 'your block')
  const shortZip = zip && zip!== 'GLOBAL'? zip : ''
  ...
  return (
    <div className="bg-white/[0.08] backdrop-blur-2xl rounded-2xl p-6 border border-white/15 shadow-2xl relative overflow-hidden">
      ...
      <div className="relative">
        ...
        {shortZip? <p className="mt-2 text- text-white/20">{shortZip}</p> : null}
      </div>
    </div>
  )
}
