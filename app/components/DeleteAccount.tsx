...
export default function DeleteAccount() {
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm('Delete your account, all posts, and unsubscribe from everything? This cannot be undone.')) return
    ...
    <div className="bg-red-950/30 border border-red-500/30 rounded-2xl p-5 mt-6">
      ...
      {show && (
        <div className="mt-4 bg-black/50 p-4 rounded-xl border border-red-500/50">
          ...
          <input value={confirmText} onChange={e=>setConfirmText(e.target.value)} placeholder="Type DELETE" className="w-full bg-white text-black rounded-xl p-2 text-sm mb-3" />
