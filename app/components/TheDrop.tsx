  ...
  return (
    <div className="bg-black/70 backdrop-blur-xl border border-white/10 rounded-2xl p-5 space-y-3">
      <div className="flex justify-between items-center">
        <h3 className="text-white font-black text-sm tracking-wider">{t?.drop?.title || 'The Drop • 10AM'}</h3>
        <span className="bg-white text-black text- font-black px-2 py-1 rounded-full">{t?.common?.live || t?.drop?.live || 'LIVE'}</span>
      </div>

      {loading? (
        <p className="text-white/50 text-sm">{t?.drop?.loading || "Loading today's drop..."}</p>
      ) : drop? (
        <>
          ...
          {drop.business_name && <p className="text-white/40 text-xs">{t?.drop?.from || 'From:'} {drop.business_name}</p>}
          {drop.claim_url && (
        ...
        <>
          <p className="text-white/60 text-sm">{t?.drop?.noDrop || "No drop yet today - Be first to sponsor tomorrow's drop!"}</p>
          <button className="w-full bg-white/10 border border-white/10 text-white py-3 rounded-xl font-bold text-sm">
            {t?.drop?.sponsor || 'Sponsor Tomorrow — $25'}
          </button>
        </>
      )}
    </div>
