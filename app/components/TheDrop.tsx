...
export default function TheDrop() {
  ...
  useEffect(() => {
    ...
    fetchDrop()
    const id = setInterval(fetchDrop, 5 * 60 * 1000) // auto-refresh 5m
  ...
  return (
    <div className="bg-black/70 backdrop-blur-xl border border-white/10 rounded-2xl p-5 space-y-3">
      <div className="flex justify-between items-center">
        <h3 className="text-white font-black text-sm tracking-wider">THE DROP • TODAY 10AM • {zip}</h3>
        <span className="bg-white text-black text-xs font-black px-3 py-1 rounded-full animate-pulse">LIVE</span>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {drop.business_name && <p className="text-white/40 text-xs">From: {drop.business_name} {drop.is_sponsored && '• Sponsored'}</p>}
          {drop.claim_url && (
          ...
          <p className="text-white/20 text-xs">Live in {zip} • Auto-refresh • What goes on in {zip} stays in {zip}</p>
        </>
      ) : (
        <>
          <p className="text-white/60 text-sm">No Drop yet in {zip} - Be first to sponsor tomorrow's Drop!</p>
          <button className="w-full bg-white/10 border border-white/10 text-white py-3 rounded-xl font-bold text-sm">
            Sponsor Tomorrow's Drop in {zip} — $25
          </button>
        </>
      )}
    </div>
