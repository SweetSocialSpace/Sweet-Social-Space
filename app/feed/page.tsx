...
export default function FeedPage() {
  ...
  const toggleMic = () => {
    ...
    if (isListening) {
      ...
      // AUTO-FORMAT on stop — so you don't have to click the button
      setDraft(prev => {
  ...
  const submit = async () => {
    ...
    setDraft(''); savedRef.current=''; finalRef.current=''
  }
      ...
      <main className="max-w-2xl mx-auto p-4">
        <div className="bg-white rounded-2xl p-4 shadow-xl">
          <div className="relative">
            <textarea value={draft} onChange={e=>{setDraft(e.target.value); savedRef.current=e.target.value}} placeholder="Tap mic, talk, tap stop — I keep everything and add. and?" rows={6} className="w-full rounded-xl border-2 border-gray-300 p-4 pr-14 text-black text- outline-none" />
          ...
          <p className={`mt-2 text-sm font-bold ${isListening?'text-red-600':'text-gray-600'}`}>{isListening?'🔴 Listening — keeps everything':'🎤 Mic keeps your words, auto-adds. and? when you tap ■'}</p>
