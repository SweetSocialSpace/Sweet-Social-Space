...
function FeedContent() {
  ...
  // BRAIN FIX: No English fallback needed - t is now always merged
  ...
  ], [t]) // removed language, t already changes
  ...
  }, [supabase, radius]) // FIX: removed t - was causing loop
  ...
  const deletePost = async (postId: string) => {
    if (!confirm(t.feed.delete + '?')) return
  ...
  const displayCity = currentProfile?.city || t.location.yourArea
      ...
      <div className="max-w- mx-auto px-3 xl:px-4 py-4 grid grid-cols-1 xl:grid-cols-[300px_minmax(0,1fr)_380px] 2xl:grid-cols-[320px_minmax(640px,860px)_400px] gap-4 xl:gap-5 items-start w-full justify-center">
        ...
        <div className="bg-black/50 backdrop-blur-2xl rounded-2xl border border-white/10 p-4 xl:p-6 w-full min-w-0">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-white/60 text-xs font-bold">{t.feed.near}</span>
            ...
            <select value={radius} onChange={(e)=>handleRadiusChange(parseInt(e.target.value))} className="bg-white/10 text-white rounded-full px-3 py-1 text-xs font-bold border border-white/20">
              <option value={5}>5 {t.radius.miles}</option><option value={10}>10 {t.radius.miles}</option><option value={15}>15 {t.radius.miles}</option><option value={20}>20 {t.radius.miles}</option>
            </select>
            <span className="text-white/40 text-xs">• {filtered.length} {t.feed.posts}</span>
          ...
          <div className="mt-2 text-xs text-white/40 px-1">{t.feed.postingAs} {authorName} • {hasNoZip? displayCity : displayZip} • {radius}{t.radius.miles}</div>
          ...
          <div className="space-y-3 mt-2">
            ...
            {filtered.map((p:any)=>{
              ...
              const displayBody = isEnded? (p.body||p.content||'').replace('LIVE NOW', t.feed.wasLive) : (p.body||p.content)
              return (
                <div key={p.id} className="bg-white rounded-2xl p-5 border-l-4 shadow-xl break-words">
                 <p className="text-black whitespace-pre-wrap break-words leading-6">
  {displayBody}
</p>
                  {p.tag === 'live' && p.livekit_room && <button onClick={() => setJoinLivePost(p)} className="mt-3 bg-red-600 text-white px-6 py-3 rounded-full font-bold text-sm w-full">🔴 {t.feed.joinLive}</button>}
                  {isEnded && (
                    <>
                      ...
                      <div className="mt-3 text-xs text-white bg-gray-800 rounded-full px-3 py-2 inline-block">{t.feed.wasLive} • {new Date(p.created_at).toLocaleString()} • {p.zip_code}</div>
                    </>
                  )}
                  <div className="mt-2 text-xs text-gray-400">{new Date(p.created_at).toLocaleString()} • {p.zip_code || displayZip}</div>
                  {currentUserId && p.user_id === currentUserId && <button onClick={()=>deletePost(p.id)} className="mt-2 bg-red-100 text-red-600 rounded-full px-3 py-1 text-xs font-bold">{t.feed.delete}</button>}
                </div>
              )
            })}
          </div>
