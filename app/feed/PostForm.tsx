...
export default function PostForm({ userId }: { userId: string }) {
  ...
  const startMic = useCallback(async () => {
    ...
    try {
      // This is the line that fixes "Microphone blocked"
      streamRef.current = await navigator.mediaDevices.getUserMedia({ audio: true });
    } catch (e) {
      setErr("Microphone blocked. Click the lock icon and allow Microphone, then try again.");
      return;
    }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    stopMic(); // Kills mic BEFORE posting - fixes runaway bug
