...
export function CommentComposerDialog({ value, onChange, onSubmit, placeholder, maxLength = 2000, submitLabel, disabled = false, title, dialogRows = 10, previewClassName, open: controlledOpen, onOpenChange }: CommentComposerDialogProps) {
  ...
  const finalPlaceholder = placeholder ?? t?.composer?.placeholder
  const finalSubmitLabel = submitLabel ?? t?.composer?.post
  const finalTitle = title ?? t?.composer?.writeComment
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-lg gap-4"><DialogHeader><DialogTitle>{finalTitle}</DialogTitle><DialogDescription>{t?.composer?.reviewHint}</DialogDescription></DialogHeader><textarea value={value} onChange={(e) => { try { onChange(e.target.value) } catch {} }} onKeyDown={handleKeyDown} placeholder={finalPlaceholder} maxLength={maxLength} rows={dialogRows} autoFocus className="w-full resize-none rounded-md border border-border bg-card p-3 text-sm outline-none focus:ring-2 focus:ring-primary/40" /><div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2"><MicRecorder onTranscript={(t2:string) => { try { onChange(t2) } catch {} }} /><span className="text-xs text-muted-foreground">{value.length}/{maxLength}</span></div><div className="flex gap-2"><button type="button" onClick={() => setOpen(false)} className="rounded-full border border-border px-4 py-2 text-sm font-medium hover:bg-secondary">{t?.common?.cancel}</button><button type="button" onClick={handleSubmit} disabled={disabled} className="rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50">{finalSubmitLabel}</button></div></div></DialogContent></Dialog>
    </>
  )
}
