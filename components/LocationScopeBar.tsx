  ...
  return (
    <div className="flex items-center justify-between w-full gap-2">
      <div className="flex items-center gap-2">
        <span className="text-white text-xs font-bold">{t?.location?.near}: {zip || '...'}</span>
        <select value={radius} onChange={(e)=>{ try { handleRadiusChange(Number(e.target.value)) } catch {} }} className="bg-white text-black text-xs font-bold rounded-full px-3 py-1">
          <option value={5}>5 {t?.location?.mi}</option><option value={10}>10 {t?.location?.mi}</option><option value={15}>15 {t?.location?.mi}</option><option value={20}>20 {t?.location?.mi}</option>
        </select>
      </div>
      <button onClick={()=>{ try { requestGeolocation() } catch {} }} className="bg-white text-black text-xs font-black rounded-full px-4 py-1.5">
        {loading? t?.location?.locating : zip? `${zip} • ${city}` : t?.location?.setLocation}
      </button>
    </div>
  )
}
