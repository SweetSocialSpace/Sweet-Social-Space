  ...
  return (
    <div className="bg-gradient-to-r from-red-600 to-red-500 rounded-xl p-4 mb-4">
      <div className="flex items-center gap-2 mb-3">
        <span className="bg-white text-red-600 px-2 py-0.5 rounded-full text-xs font-bold animate-pulse">{t?.live?.live}</span>
        <span className="text-white font-bold text-sm">{t?.live?.liveNow}</span>
      </div>
        ...
        {livePosts.map((post) => (
          <div key={post.id} className="bg-white/10 backdrop-blur rounded-lg p-3 min-w- flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <span className="bg-red-500 text-white px-2 py-0.5 rounded text-xs font-bold animate-pulse">● {t?.live?.live}</span>
