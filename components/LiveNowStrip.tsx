  ...
  const toggleLive = async ()=>{
    if(!isLive){
      try{
        ...
        await supabase.from('live_streams').insert({ user_id: user?.id, is_active:true, status:'live', zip:'95122' })
  ...
  return (
    <div className="flex items-center justify-between w-full">
      <div className="flex items-center gap-2 text-white text-xs">
        ...
        <span className="text-white/70">{isLive?'You are live!':'No one live — start one!'}</span>
      </div>
