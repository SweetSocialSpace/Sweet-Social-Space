  ...
  useEffect(() => {
    ...
    const loadRealWorld = async () => {
      ...
      try {
        ...
        if (temp && temp >= 90) {
          if (mounted) setAlert({ title: `Heat Advisory • ${zip}`, body: `High heat ${Math.round(temp)}°F in ${zip} right now - Be cautious. Live from NWS ${city || zip}` })
          return
        }
        if (mounted) setAlert(null)
      } catch { if (mounted) setAlert(null) }
    }
    if (!zip) return
    loadRealWorld()
