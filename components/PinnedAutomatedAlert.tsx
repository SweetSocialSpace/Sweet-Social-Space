...
export function PinnedAutomatedAlert() {
  ...
  useEffect(()=>{
    ...
    const run = async () => {
      ...
      const { data: recent } = await supabase.from('posts').select('id,body,tag,created_at,user_id').eq('zip_code', zip).gte('created_at', since24).order('created_at',{ascending:false}).limit(20)
      const automated = (recent||[]).find((r:any)=>botIds.has(r.user_id)) as AlertPost | undefined
      if(automated) setAlert(automated)
    }
    run()
  },[zip])
    ...
    <div className="bg-black/40 backdrop-blur-xl rounded-2xl p-5 border border-white/10 text-white">
      ...
      {alert? (
        <>
          ...
          <p className="text- mt-2 text-white/50">{new Date(alert.created_at).toLocaleString()} • {zip}</p>
        </>
      ) : (
