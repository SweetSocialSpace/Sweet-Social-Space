...
export function SidebarTabs({ tabs, initial }: { tabs: SidebarTab[]; initial?: string }) {
  ...
  return (
    <div className="space-y-3">
        ...
        {tabs.map((t) => {
          ...
          return (
            <button key={t.id} role="tab" aria-selected={isActive} onClick={() => { try { setActive(t.id) } catch {} }} title={t.label}
              className={`flex min-w-0 items-center justify-center rounded-xl px-1 py-1.5 text-xs font-semibold transition ${isActive? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
      ...
      <div role="tabpanel" className="space-y-3">{current?.content}</div>
    </div>
  )
}
