...
export function MarketplacePreview(){
  ...
  useEffect(()=>{
    ...
    let mounted = true
    const load = async()=>{
      const {data} = await supabase.from('marketplace').select('id,title,price,address,sale_date,sale_time').order('created_at',{ascending:false}).limit(10)
            ...
            <div key={i.id} className="bg-white/5 rounded-xl p-3">
              ...
              {i.address && (
                <div className="mt-2.5">
                  <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(i.address)}`} target="_blank" className="inline-flex text- bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-full">📍 Map & Directions</a>
                </div>
              )}
            </div>
          ))}
