...
export function LegalLayout({ title, updated, children }: { title: string; updated: string; children: ReactNode }) {
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui, sans-serif', position: 'relative' }}>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: -1, background: 'rgba(255,255,255,0.82)', backdropFilter: 'blur(2px)' }} />
      ...
      <main style={{ maxWidth: '768px', margin: '0 auto', width: '100%', flex: 1, padding: '48px 24px' }}>
        ...
        <div style={{ lineHeight: 1.8, fontSize: 15, color: '#222', background: 'rgba(255,255,255,0.9)', padding: '24px', borderRadius: '12px' }}>{children}</div>
      </main>
      <footer style={{ borderTop: '1px solid rgba(0,0,0,0.08)', padding: '20px', textAlign: 'center', fontSize: 12, color: '#666', background: 'rgba(255,255,255,0.85)' }}>
        © Sweet Social Space • GLOBAL • SSL SECURED • <Link href="/legal/terms" style={{ marginLeft: 8, color: '#555' }}>Terms of Service</Link> • <Link href="/legal/privacy" style={{ marginLeft: 8, color: '#555' }}>Privacy</Link> • <Link href="/legal/contact" style={{ marginLeft: 8, color: '#555' }}>Contact</Link>
      </footer>
    </div>
  )
}
