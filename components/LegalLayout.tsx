    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      <header style={{ borderBottom: '1px solid rgba(255,255,255,0.15)', padding: '20px', background: 'rgba(0,0,0,0.32)' }}>
        <div style={{ maxWidth: '768px', margin: '0 auto', display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Link href="/feed" style={{ fontSize: '14px', color: '#ddd', textDecoration: 'none' }}>{t?.legal?.feed || 'Feed'} →</Link>
        </div>
      </header>
      <main style={{ maxWidth: '768px', margin: '0 auto', width: '100%', flex: '1', padding: '48px 24px' }}>
        ...
        <p style={{ fontSize: '13px', color: '#bbb', marginBottom: '32px' }}>{t?.legal?.lastUpdated || 'Last updated:'} {updated}</p>
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.1)', padding: '20px', textAlign: 'center', fontSize: '12px', color: '#aaa', background: 'rgba(0,0,0,0.32)' }}>
       © Sweet Social Space • {t?.legal?.sslSecured || 'SSL SECURED'} • <Link href="/legal/terms" style={{ marginLeft: '8px', color: '#ccc' }}>{t?.legal?.terms || 'Terms'}</Link> • <Link href="/legal/privacy" style={{ marginLeft: '8px', color: '#ccc' }}>{t?.legal?.privacy || 'Privacy'}</Link> • <Link href="/legal/contact" style={{ marginLeft: '8px', color: '#ccc' }}>{t?.legal?.contact || 'Contact'}</Link>
      </footer>
    </div>
  )
}
