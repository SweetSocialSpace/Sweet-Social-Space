...
export function ProfileDialog({ onClose }: { onClose: () => void }) {
  ...
  const removeAvatar = async () => {
    ...
    try {
      ...
      setAvatarUrl(null); setPreviewUrl(null); setMsg('Profile picture removed.')
  ...
  const save = async () => {
    try {
      ...
      if (name.length < 2 || name.length > 40) { setErr('Display name must be 2–40 characters.'); return }
      setBusy(true)
      try {
        const supabase = createClient() as any
        let u: any = null; try { const { data } = await supabase.auth.getUser(); u = data.user } catch {}
        ...
        if (error) {
          try { const res2 = await supabase.from('profiles').update({ display_name: name, bio: bio.trim() || null }).eq('user_id', u.id); if (!res2.error) { setMsg('Profile saved.'); return } error = res2.error } catch {}
        }
        if (error) { setErr((error as any).message || 'Save failed'); return }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-xl" onClick={(e) => e.stopPropagation()} role="dialog" aria-labelledby="profile-dialog-title">
        <h2 id="profile-dialog-title" className="font-display text-lg font-semibold">Edit profile</h2>
        <p className="mt-1 text-xs text-muted-foreground">This is how neighbors see you across Sweet Social Space.</p>
        <div className="mt-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <button type="button" onClick={() => { try { fileRef.current?.click() } catch {} }} disabled={uploading} className="rounded-full border border-border px-3 py-1.5 text-xs font-medium hover:bg-secondary disabled:opacity-50">{uploading? 'Uploading…' : avatarUrl? 'Change photo' : 'Upload photo'}</button>
              {avatarUrl && (<button type="button" onClick={()=>{ try { removeAvatar() } catch {} }} disabled={uploading} className="rounded-full border border-border px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-secondary disabled:opacity-50">Remove</button>)}
            </div>
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          ...
          <button onClick={()=>{ try { save() } catch {} }} disabled={busy} className="rounded-full px-5 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50" style={{ background: 'var(--gradient-warm)', boxShadow: 'var(--shadow-sweet)' } as any}>{busy? 'Saving…' : 'Save'}</button>
        </div>
      </div>
    </div>
  )
