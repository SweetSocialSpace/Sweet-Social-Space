...
export function NotificationBell({ className = '' }: { className?: string }) {
  ...
  return (
    <Link href="/notifications" aria-label={count? `${count} ${t?.notifications?.unreadNotifications || 'unread notifications'}` : (t?.notifications?.notifications || 'Notifications')} className={`relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-border bg-card text-foreground hover:bg-secondary ${className}`}>
