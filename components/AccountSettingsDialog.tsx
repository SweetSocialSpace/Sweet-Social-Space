...
export function AccountSettingsDialog({ onClose }: { onClose: () => void }) {
  ...
  const exportData = async () => {
    ...
    } catch (e: any) { setMsg(e.message || 'Export failed') } finally { setBusy(false) }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mt-1 text-xs text-muted-foreground">Under GDPR and CCPA you can export your data or delete your account at any time.</p>
        <div className="mt-4 space-y-2"><button onClick={()=>{ try { exportData() } catch {} }} disabled={busy} className="w-full rounded-xl border border-border px-4 py-2 text-sm hover:bg-secondary disabled:opacity-50">Export all my data (JSON)</button></div>
        <div className="mt-6 rounded-xl border border-red-200 bg-red-50 p-3 dark:border-red-900/40 dark:bg-red-950/30"><p className="text-sm font-medium text-red-900 dark:text-red-200">Delete my account</p><p className="mt-1 text-xs text-red-800/80 dark:text-red-200/80">We’ll erase your account, posts, likes, and profile within 30 days. Some records (billing, abuse reports) may be kept longer if required by law. Type <strong>DELETE</strong> to confirm.</p><input value={confirmDelete} onChange={(e) => { try { setConfirmDelete(e.target.value) } catch {} }} placeholder="Type DELETE" className="mt-2 w-full rounded-lg border border-red-300 bg-background px-3 py-2 text-sm" /><button onClick={()=>{ try { requestDeletion() } catch {} }} disabled={busy || confirmDelete!== 'DELETE'} className="mt-2 w-full rounded-full bg-red-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">Request deletion</button></div>
        {msg && <p className="mt-3 text-sm text-foreground">{msg}</p>}
        <div className="mt-5 flex items-center justify-between"><button onClick={() => { try { window.dispatchEvent(new Event('open-cookie-settings')) } catch {} }} className="text-xs text-muted-foreground underline">Cookie settings</button><button onClick={onClose} className="rounded-full border border-border px-4 py-2 text-sm hover:bg-secondary">Close</button></div>
      </div>
    </div>
  )
}
