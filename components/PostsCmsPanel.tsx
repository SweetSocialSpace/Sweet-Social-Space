...
export function PostsCmsPanel({ title = 'Posts CMS', description, userId, userIds, scope, zip_code, showSearch = true }: Props) {
  ...
  function visibilityBadge(v: PostVisibility) { if (v=== 'published') return <Badge className="bg-green-600">{t?.cms?.published}</Badge>; if (v=== 'draft') return <Badge variant="secondary">{t?.cms?.draft}</Badge>; return <Badge variant="outline">{t?.cms?.unpublished}</Badge> }
  ...
  const saveEdit = async () => {
    if (!editingId) return; if (!editBody.trim()) { toast.error(t?.cms?.bodyCannotBeEmpty); return }
    setBusy(true)
    try { await updateAdminPost({ id: editingId, body: editBody.trim(), tag: editTag.trim() }); toast.success(t?.cms?.postUpdated); setEditingId(null); invalidate() } catch (err: any) { toast.error(err?.message?? t?.cms?.failedToUpdate) } finally { try { setBusy(false) } catch {} }
  }
  const onDelete = async (id: string) => { try { await deleteAdminPost({ id }); toast.success(t?.cms?.postDeleted); if (editingId=== id) setEditingId(null); invalidate() } catch (err: any) { toast.error(err?.message?? t?.cms?.failedToDelete) } }
  const onSetVis = async (id: string, visibility: PostVisibility) => { try { await setAdminPostVisibility({ id, visibility }); toast.success(`${t?.cms?.marked} ${visibility}`); invalidate() } catch (err: any) { toast.error(err?.message?? t?.cms?.failed) } }

  return (
    <section className="space-y-6">
      <header><h2 className="font-display text-xl font-semibold">{title || t?.cms?.title}</h2>{description && (<p className="mt-1 text-sm text-muted-foreground">{description}</p>)}</header>
      {showSearch && (<Input placeholder={t?.cms?.searchPlaceholder} value={search} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)} maxLength={120} />)}
      <div className="space-y-3">
        <h3 className="text-sm font-semibold">{t?.cms?.posts} ({posts.length}) {zip_code? `• ${zip_code}`:''}</h3>
        {isLoading && <p className="text-sm text-muted-foreground">{t?.common?.loading} </p>}
        {!isLoading && posts.length=== 0 && (<p className="rounded-xl border border-dashed border-border p-4 text-sm text-muted-foreground">{t?.cms?.noPostsMatch}</p>)}
        {posts.map((p) => {
          const isEditing = editingId=== p.id
          return (
            <article key={p.id} className="rounded-xl border border-border bg-card p-4">
              <div className="flex flex-wrap items-center gap-2">{visibilityBadge(p.visibility)}{p.author_is_bot && <Badge variant="outline">🤖 {t?.cms?.bot}</Badge>}{p.tag && <Badge variant="outline">{p.tag}</Badge>}{p.zip_code && (<Badge variant="outline">ZIP {p.zip_code}</Badge>)}<span className="text-xs text-muted-foreground">{p.author_name?? p.user_id.slice(0, 8)}</span><span className="ml-auto text-xs text-muted-foreground">{new Date(p.created_at).toLocaleString()}</span></div>
              {isEditing? (
                <div className="mt-3 space-y-3"><label className="block text-sm"><span className="text-muted-foreground">{t?.cms?.tag}</span><Input value={editTag} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditTag(e.target.value)} maxLength={40} /></label><label className="block text-sm"><span className="text-muted-foreground">{t?.cms?.body} ({editBody.length}/2000)</span><Textarea value={editBody} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setEditBody(e.target.value)} maxLength={2000} rows={6} /></label><div className="flex gap-2"><Button size="sm" onClick={saveEdit} disabled={busy}>💾 {t?.cms?.save}</Button><Button size="sm" variant="ghost" onClick={() => setEditingId(null)}>{t?.common?.cancel}</Button></div></div>
              ) : (<pre className="mt-2 whitespace-pre-wrap text-sm">{p.body}</pre>)}
              {!isEditing && (<div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => beginEdit(p)}>✏ {t?.cms?.edit}</Button>{p.visibility!== 'published' && (<Button size="sm" onClick={() => onSetVis(p.id, 'published')}>📢 {t?.cms?.publish}</Button>)}{p.visibility=== 'published' && (<Button size="sm" variant="secondary" onClick={() => onSetVis(p.id, 'unpublished')}>🙈 {t?.cms?.unpublish}</Button>)}{p.visibility!== 'draft' && (<Button size="sm" variant="ghost" onClick={() => onSetVis(p.id, 'draft')}>📝 {t?.cms?.markAsDraft}</Button>)}<AlertDialog><AlertDialogTrigger><Button size="sm" variant="destructive">🗑 {t?.cms?.delete}</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{t?.cms?.deleteThisPost}</AlertDialogTitle><AlertDialogDescription>{t?.cms?.deleteDesc}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{t?.common?.cancel}</AlertDialogCancel><AlertDialogAction onClick={() => onDelete(p.id)}>{t?.cms?.delete}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div>)}
            </article>
          )
        })}
      </div>
      <div className="space-y-3"><h3 className="text-sm font-semibold">{t?.cms?.auditLog} ({audit.length})</h3><div className="rounded-xl border border-border bg-card">{audit.length=== 0 && (<p className="p-4 text-sm text-muted-foreground">{t?.cms?.noActionsYet}</p>)}{audit.map((row) => (<div key={row.id} className="border-b border-border p-3 text-sm last:border-b-0"><div className="flex flex-wrap items-center gap-2"><Badge variant="outline" className="text-xs uppercase">{row.action}</Badge><span className="font-medium">{row.actor_is_bot? '🤖 ' : ''}{row.actor_name?? row.actor_id?.slice(0, 8)?? 'system'}</span><span className="text-xs text-muted-foreground">{new Date(row.created_at).toLocaleString()}</span>{row.post_id && (<span className="ml-auto font-mono text-xs text-muted-foreground">{row.post_id.slice(0, 8)}</span>)}</div>{row.snapshot?.body && (<p className="mt-1 line-clamp-2 text-xs text-muted-foreground">"{String(row.snapshot.body).slice(0, 160)}"</p>)}</div>))}</div></div>
    </section>
  )
}
