...
export function EmergencyAlerts() {
  ...
  if (!globalZip) return (<div className="bg-black/40 backdrop-blur-xl rounded-2xl p-5 border border-white/10 text-white"><p className="font-bold">🚨 {t?.emergency?.title || 'Emergency'} • {t?.emergency?.loadingLocation || 'Loading location...'}</p></div>)

  return (
    <div className="bg-black/40 backdrop-blur-xl rounded-2xl p-5 border border-white/10 text-white">
      <p className="font-bold flex items-center gap-2">🚨 {t?.emergency?.title || 'Emergency'} • {t?.emergency?.near || 'Near'} {zip}</p>
      {status === 'checking' && <p className="text-sm mt-2 text-white/60 animate-pulse">{t?.emergency?.scanning || 'Scanning NOAA, OpenWeather, USGS...'}</p>}
      {status === 'clear' && (<div className="mt-2">{alerts.map(a=> (<p key={a.id} className="text-sm text-white/80 bg-white/5 rounded-lg p-2.5">{(a as any).icon} {a.title} - {a.message}</p>))}{alerts.length===0 && <p className="text-sm mt-1 text-white/80">{t?.emergency?.allClearIn || 'All clear in'} {zip} • {t?.emergency?.noActive || 'No active alerts'}</p>}</div>)}
      {status === 'alert' && (<div className="mt-3 space-y-2">{alerts.map(a=> (<div key={a.id} className="text-sm bg-red-500/15 border border-red-500/30 rounded-lg p-2.5"><div className="font-black text-red-200 text-xs">{(a as any).icon || '🚨'} {a.title || a.event} {a.type? `• ${a.type}` : ''}</div><div className="text-white/80 mt-1 text-xs leading-snug">{a.message || a.body}</div></div>))}<p className="text-xs text-white/30 mt-2">{t?.emergency?.autoRefresh || 'Auto-refresh 5m • Weather & Safety APIs'}</p></div>)}
    </div>
  )
}
