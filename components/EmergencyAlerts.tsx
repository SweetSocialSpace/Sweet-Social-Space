...
export function EmergencyAlerts() {
  ...
  useEffect(()=>{
    ...
    const load = async()=>{
      try{
        ...
        if(mounted && count && count>0){ setAlerts((data as any) || []); setStatus('alert'); return }
  ...
  return (
    <div className="bg-black/40 backdrop-blur-xl rounded-2xl p-5 border border-white/10 text-white">
      ...
      {status === 'alert' && (<div className="mt-3 space-y-2">{alerts.map(a=> (<div key={a.id} className="text-sm bg-red-500/15 border border-red-500/30 rounded-lg p-2.5"><div className="font-black text-red-200 text-xs">{(a as any).icon || '🚨'} {a.title || a.event} {a.type? `• ${a.type}` : ''}</div><div className="text-white/80 mt-1 text-xs leading-snug">{a.message || a.body}</div></div>))}<p className="text-xs text-white/30 mt-2">Live from OpenWeather, NOAA, USGS • auto-refresh 5m</p></div>)}
    </div>
  )
}
