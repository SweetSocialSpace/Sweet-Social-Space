...
export function LatestAlerts({ compact = false }: { compact?: boolean }) {
              ...
              <li key={alert.id}>
                ...
                >
                  <div className={`flex min-w-0 items-center gap-1.5 ${compact? 'flex-wrap' : ''}`}>
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text- font-medium uppercase tracking-wide ${SEVERITY_STYLE[alert.severity]?? SEVERITY_STYLE.info}`}>
                      {alert.severity}
                    </span>
                    {!compact && (
                      <span className="shrink-0 rounded-full border border-border px-2 py-0.5 text- font-medium uppercase tracking-wide text-muted-foreground">
                  ...
                  <p className="mt-2 text- font-medium text-muted-foreground">
